...
from hr.helpers import parse_date
...
def compare_snapshots(data1: dict, data2: dict) -> dict:
    ...
    Quelle: app.py Zeilen 4712-4762
    ...
    return {
        'added': [
            {
                'pid': pid,
                'name': f"{data2[pid].get('core', {}).get('Vorname', '')} {data2[pid].get('core', {}).get('Nachname', '')}".strip()
            }
        ...
        'removed': [
            {
                'pid': pid,
                'name': f"{data1[pid].get('core', {}).get('Vorname', '')} {data1[pid].get('core', {}).get('Nachname', '')}".strip()
            }
...
def build_employee_history(snapshots: list[dict]) -> dict:
    ...
    Quelle: app.py Zeilen 2676-2778

    Ersetzt _get_employee_history_from_snapshots() – statt Dateisystem
    werden die Snapshots direkt als Liste von Dicts übergeben.

    Args:
        snapshots: Liste von Snapshots, sortiert nach Zeitstempel (ältester zuerst).
