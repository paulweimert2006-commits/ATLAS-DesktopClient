...
class BiPROView(QWidget):
    ...
    def _setup_ui(self):
        ...
        from i18n.de import (
            ...
            ACC_BIPRO_REFRESH, ACC_BIPRO_VIEW_TOGGLE, ACC_BIPRO_FETCH_ALL,
            ACC_BIPRO_MAIL_FETCH, ACC_BIPRO_FETCH_SINGLE, ACC_BIPRO_ACK_ALL
        )
        ...
        self._refresh_btn = QPushButton(f"🔃 {BIPRO_PREVIEW_REFRESH}")
        self._refresh_btn.setFixedHeight(28)
        self._refresh_btn.setStyleSheet(get_button_ghost_style())
        self._refresh_btn.setToolTip(f"{BIPRO_PREVIEW_REFRESH} (max. 1x / 30s) (Ctrl+R)")
        self._refresh_btn.setShortcut("Ctrl+R")
        self._refresh_btn.setAccessibleName(ACC_BIPRO_REFRESH)
        self._refresh_btn.clicked.connect(self._on_manual_refresh)
        header_row.addWidget(self._refresh_btn)

        header_row.addStretch()

        self._admin_toggle = QPushButton(f"👤 {BIPRO_VIEW_TOGGLE_STANDARD}")
        ...
        self._admin_toggle.setToolTip(f"{ACC_BIPRO_VIEW_TOGGLE} (Ctrl+T)")
        self._admin_toggle.setAccessibleName(ACC_BIPRO_VIEW_TOGGLE)
        ...
        self.fetch_all_vus_btn.setAccessibleName(ACC_BIPRO_FETCH_ALL)
        ...
        self.mail_fetch_btn.setAccessibleName(ACC_BIPRO_MAIL_FETCH)
        ...
        self.fetch_single_vu_btn.setAccessibleName(ACC_BIPRO_FETCH_SINGLE)
        ...
        self._ack_btn.setAccessibleName(ACC_BIPRO_ACK_ALL)
        self._ack_btn.setToolTip(
            f"Quittiert ALLE gelisteten Lieferungen bei allen Versicherern (Ctrl+Alt+A).\n"
    ...
    def _toggle_view_mode(self, checked: bool):
        ...
        # Emojis je nach Modus: 🛠️ fuer Admin-Ansicht, 👤 fuer Standard-Ansicht
        prefix = "🛠️" if checked else "👤"
        self._admin_toggle.setText(
            f"{prefix} {BIPRO_VIEW_TOGGLE_ADMIN if checked else BIPRO_VIEW_TOGGLE_STANDARD}"
        )
        self._apply_view_mode()
    ...
    def _apply_view_mode(self):
        ...
        for w in self._standard_only_widgets:
            ...
            # Deaktiviere Widgets im Standard-Modus wenn sie versteckt sind,
            # um versehentliche Shortcut-Trigger (z.B. Ctrl+Alt+A) zu verhindern.
            w.setEnabled(not admin)
    ...
    def _on_ack_parallel_worker_done(self, vu_name: str, ok_ids: list,
        ...
        self._ack_btn.setText(
            f"✅ {BIPRO_ACK_IN_PROGRESS.format(
