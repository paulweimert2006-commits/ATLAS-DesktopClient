...
ANPASSUNG gegenueber HR-Hub:
- Kein TriggerStore/TriggerLogStore (JSON-basiert) mehr
- Trigger-Config und Logs kommen/gehen ueber den API-Client
...
class TriggerEngine:
    """
    Engine zur Auswertung von Triggern und Ausfuehrung von Aktionen.

    Wird beim Delta-Export aufgerufen und prueft:
    1. Welche Trigger fuer den Arbeitgeber aktiv sind
    2. Welche Events eingetreten sind
    3. Ob die Bedingungen erfuellt sind
    4. Fuehrt die Aktionen aus (E-Mail, API)
    """
    ...
    def evaluate_and_execute(self, employer_cfg: dict, diff: dict,
        ...
        """
        Wertet alle Trigger aus und fuehrt passende Aktionen aus.
        ...
        Args:
            ...
            smtp_config: SMTP-Konfiguration fuer E-Mail-Versand
            executed_by: Ausfuehrender Benutzer

        Returns:
            Liste der Ausfuehrungsergebnisse
        """
        ...
        for trigger in triggers:
            try:
                ...
                action_type = trigger.get('action_type', 'email')
                ...
                results.append({
                    ...
                    'employer_name': employer_name,
                    'affected_employees': [],
                    'action_type': trigger.get('action_type', 'unknown'),
                    'action_details': {},
                    'status': 'error',
                    'error_message': str(e),
                    'executed_by': executed_by,
    ...
    def _process_changed_employees(self, changed_list, current_data, trigger):
        conditions = trigger.get('conditions_json', []) or trigger.get('conditions', [])
    ...
    def _check_condition(self, condition, changes):
        ...
        if operator == 'changed':
            ...
            new_val = change.get('to', change.get('new', '')) if isinstance(change, dict) else ''
            return change is not None and safe_str(new_val).lower() == safe_str(to_value).lower()
        elif operator == 'changed_from':
            old_val = change.get('from', change.get('old', '')) if isinstance(change, dict) else ''
            return change is not None and safe_str(old_val).lower() == safe_str(from_value).lower()
        elif operator == 'changed_from_to':
            if change is None:
                return False
            old_val = change.get('from', change.get('old', '')) if isinstance(change, dict) else ''
            new_val = change.get('to', change.get('new', '')) if isinstance(change, dict) else ''
            return (safe_str(old_val).lower() == safe_str(from_value).lower() and
                    safe_str(new_val).lower() == safe_str(to_value).lower())
        elif operator == 'is_empty':
            new_val = change.get('to', change.get('new')) if isinstance(change, dict) else None
            return change is not None and not new_val
        elif operator == 'is_not_empty':
            new_val = change.get('to', change.get('new')) if isinstance(change, dict) else None
            return change is not None and new_val
        elif operator == 'contains':
            new_val = change.get('to', change.get('new', '')) if isinstance(change, dict) else ''
            return (change is not None and to_value and
                    safe_str(to_value).lower() in safe_str(new_val).lower())
        return False
    ...
    def _build_context(self, employer_cfg, trigger, affected_employees, current_data):
        ...
        if len(affected_employees) == 1:
            ...
            if emp.get('changes'):
                first_change = emp['changes'][0] if isinstance(emp['changes'], list) else {}
                context['_changedField'] = first_change.get('field', '')
                context['_oldValue'] = first_change.get('old', first_change.get('from', ''))
                context['_newValue'] = first_change.get('new', first_change.get('to', ''))
            all_changes = []
            changes = emp.get('changes', [])
            if isinstance(changes, list):
                for c in changes:
                    all_changes.append(f"{c.get('field')}: {c.get('old', c.get('from', '-'))} -> {c.get('new', c.get('to', '-'))}")
            context['_allChanges'] = '\n'.join(all_changes)
...
class EmailAction:
    """Handler fuer E-Mail-Aktionen mit Mustache-Template-Rendering."""
    ...
    def execute(self, config, context, smtp_config):
        ...
        if not recipients:
            return False, {}, "Keine Empfaenger angegeben"
...
class APIAction:
    """Handler fuer API-Aktionen (HTTP-Requests)."""
