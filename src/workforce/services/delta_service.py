...
from workforce.helpers import (
    json_hash, flatten_record, person_key, getv, get_safe_employer_name
)
from workforce.services.export_service import map_to_scs_schema, generate_delta_excel
from workforce.services.snapshot_service import compare_snapshots
...
def build_current_snapshot_data(current_details: list[dict], employer_cfg: dict) -> dict:
    ...
    for detail in current_details:
        ...
        leave_date_str = getv(detail, details_dict, "Kuendigungsdatum", "leaveDate", "termination_date", "contract_end_date")
...
def calculate_diff(current: dict, previous: dict) -> tuple[dict, set, set]:
    """
    ...
    Returns:
        Tupel (diff_dict, added_pids, changed_pids)
    """
...
def generate_delta_export(current_details: list[dict], employer_cfg: dict,
    ...
    Args:
        ...
        exports_dir: Lokales Verzeichnis fuer temporaere Export-Dateien
